...
(function($) {
	...
	tds.each(function(a) {
		...
		switch(th.attr("data-tsort-type")){
		    ...
		    case "number": text = parseFloat($(this).find('input[type=hidden]').val().replace(/[,]/g, ".").replace(/[^0-9\.\-]/g, "")); break;
		    case "date": try { text = new Date($(this).text()).getTime(); } catch (err) { text = 0 }; break;
			
			case "date-br" : 
			
			    if ($(this).find('input[type=hidden]').val().match(/[0-9\/]+[\s]+[0-9]+[:]/g)) //Datetime
			        text = ($(this).find('input[type=hidden]').val().split(" ")[0].split("/").reverse().join("-")) + " " + ($(this).find('input[type=hidden]').val().split(" ")[1]);
			    else if ($(this).find('input[type=hidden]').val().match(/[0-9]{2}[\/]{1}[0-9]{2}[\/]{1}[0-9]{4}/g))
			        text = $(this).find('input[type=hidden]').val().split("/").reverse().join("-");
				else
					text = 0;
